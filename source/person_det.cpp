	...
	while(1)
	{
		...
		for (const auto& result: results) {
			if(result.m_normalisedVal > BOX_SCORE_THRESHOLD) //score of box
			{
				...
				if (s_odRets[s_odRetCnt].x2 > CAMERA_WIDTH-10)
					s_odRets[s_odRetCnt].x2 = CAMERA_WIDTH-10;
				if (s_odRets[s_odRetCnt].y2 > CAMERA_HEIGHT-10)
					s_odRets[s_odRetCnt].y2 = CAMERA_HEIGHT-10;
